...
bool SceneStage1::init()
{
    ...
    // SpawnMonster* pSpawnMonster2 = createObject<SpawnMonster>(L"SpawnMonster2", Vec(600.f, 100.f));
    ...
    pTileMap->loadFile("Stage1.map");
    pTileMap->createObj();

    getCamera()->fadeIn(5.f);

	return true;
}

bool SceneStage1::update()
{
	...
	{
		if ((*beginIter)->getObjName() == L"KamikazeLiteSpawn")
		{
			++m_uSpawnCount;
		}
		++beginIter;
...
bool SceneStage1::postUpdate()
{
    ...
    if (0 == m_uSpawnCount && false == m_bPotalCreated)
    {
        Potal* pPotal = createObject<Potal>(L"Potal", Vec(400.f, 400.f));
        getSceneResource()->playSound(L"EffectPotal");
        m_bPotalCreated = true;
    }

	return true;
}

bool SceneStage1::collision()
{
...
bool SceneStage1::render(HDC _hDC)
{
...
void SceneStage1::loadSound()
{
    getSceneResource()->loadSound(L"Effect", false, L"EffectHit", "effect_hit.mp3");
    getSceneResource()->loadSound(L"Effect", false, L"EffectPotal", "effect_potal.wav");
}

void SceneStage1::loadAnimationSequence()
{
    /* Player Animation */
...
void SceneStage1::load(const wstring& _strName, const wstring& _strPath, int _nFrameCount, float _fSizeX, float _fSizeY)
{
